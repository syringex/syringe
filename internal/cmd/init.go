...
// then runs its own `init` step if it has no persisted config yet (or
// --force). A tag-shaped value with no known provider identity is left
// alone entirely — it's presumed a literal, not a typo.
func runInit(cmd *cobra.Command, f *flags, force bool) error {
	...
	for _, name := range requiredProviderNames(entries) {
		...
		if binaryInstalled && !force {
			...
			fmt.Fprintf(out, "%s: building from %s\n", name, pkg)
		...
		if force || !providerdir.HasConfig(dir, name) {
			...
			if err != nil {
				...
				// configured, or skip .gitignore housekeeping below for
				// what did succeed.
